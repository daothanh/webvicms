      ...
      $(function () {
        ...
        $('#delete-all').click(function () {
          ...
          if (ids.length) {
            swal({
              title: '{{ __("Are you sure?") }}',
              text: "{{ __("All selected pages will be moved to the trash!") }}",
              type: 'warning',
              showCancelButton: true,
              confirmButtonColor: '#3085d6',
              cancelButtonColor: '#d33',
              ...
              if (result.value) {
                axios.post("{{ route('api.page.delete-multiple') }}", {ids: ids})
                  .then(function (rs) {
                    pageTable.ajax.reload();
                    notify('{{ __("Notification") }}', '{{ __("All selected pages were moved to the trash!") }}', 'success');
                  })
                  .catch(function (error) {
                    notify('{{ __("Alert") }}', error.response.data.message, 'error');
                  });
        ...
        $('#restore-all').click(function () {
          ...
          if (ids.length) {
            swal({
              title: '{{ __("Are you sure?") }}',
              text: "{{ __("All selected pages will be restored!") }}",
              type: 'warning',
              showCancelButton: true,
              confirmButtonColor: '#3085d6',
              cancelButtonColor: '#d33',
              ...
              if (result.value) {
                axios.post("{{ route('api.page.restore-multiple') }}", {ids: ids})
                  .then(function (rs) {
                    pageTable.ajax.reload();
                    notify('{{ __("Notification") }}', '{{ __("All selected pages were restored!") }}', 'success');
                  })
                  .catch(function (error) {
                    notify('{{ __("Alert") }}', error.response.data.message, 'error');
                  });
