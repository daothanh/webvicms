    ...
    <div class="card">
        <div class="card-header">
            ...
            <div class="card-tools">
                <a href="{{ route('admin.page.create') }}" class="btn"><i
                        class="icon ion-md-add"></i> {{ __('page::page.title.Create a page') }}</a>
            </div>
        </div>
        <div class="card-body">
            ...
            <table id="data-table" class="table table-striped">
                ...
                <tr>
                    ...
                    <th>{{ __('page::page.labels.Title') }}</th>
        ...
        $(function () {
            var pageTable = $('#data-table').DataTable({
                ...
                createdRow: function (row, data, dataIndex) {
                    $(row).find('td:eq(4)').html('<div class="custom-control custom-switch">\n' +
            ...
            $('body').on('click', '.del-checkbox', function () {
                ...
                swal({
                    title: '{{ __("page::page.title.Alert") }}',
                    text: "{{ __("page::page.messages.It will be moved to the trash") }}",
                    type: 'warning',
                    showCancelButton: true,
                    confirmButtonColor: '#3085d6',
                    cancelButtonColor: '#d33',
                    cancelButtonText: '{{ __("page::page.labels.Cancel") }}',
                    confirmButtonText: '{{ __("page::page.labels.Yes, delete it") }}'
                }).then((result) => {
                    if (result.value) {
                        axios.delete(url)
                            .then(rs => {
                                if (rs.data.error === false) {
                                    ...
                                    notify('{{ __("page::page.title.Notification") }}', '{{ __("page::page.messages.It was moved to the trash") }}', 'success');
                                } else {
                                    notify('{{ __("page::page.title.Alert") }}', rs.data.error, 'error');
                                }
                            });
                    }
                });
                ...
                swal({
                    title: '{{ __("page::page.title.Alert") }}',
                    text: "{{ __("page::page.messages.It will be restored") }}",
                    type: 'warning',
                    showCancelButton: true,
                    confirmButtonColor: '#3085d6',
                    cancelButtonColor: '#d33',
                    cancelButtonText: '{{ __("page::page.labels.Cancel") }}',
                    confirmButtonText: '{{ __("page::page.labels.Yes, restore it") }}'
                }).then((result) => {
                    if (result.value) {
                        axios.post(url)
                            .then(rs => {
                                if (rs.data.error === false) {
                                    pageTable.ajax.reload();
                                    notify('{{ __("page::page.title.Notification") }}', '{{ __("page::page.messages.It was restored") }}', 'success');
                            ...
                            .catch(function (error) {
                                notify('{{ __("page::page.title.Alert") }}', error.response.data.message, 'error');
                            });
                    }
                });
            }).on('click', '.toggle-status', function () {
                var checked = $(this).prop('checked');
                axios.post('{{ route("api.page.toggle_status") }}', {id: $(this).data('id')});
            });
            ...
            $('#delete-all').click(function () {
                ...
                if (ids.length) {
                    swal({
                        title: '{{ __("page::page.title.Alert") }}',
                        text: "{{ __("page::page.messages.All selected pages will be moved to the trash") }}",
                        type: 'warning',
                        showCancelButton: true,
                        confirmButtonColor: '#3085d6',
                        cancelButtonColor: '#d33',
                        ...
                        if (result.value) {
                            axios.post("{{ route('api.page.delete-multiple') }}", {ids: ids})
                                .then(function (rs) {
                                    pageTable.ajax.reload();
                                    notify('{{ __("page::page.title.Notification") }}', '{{ __("page::page.messages.All selected pages were moved to the trash") }}', 'success');
                                    $('#mass-select-all').removeAttr('checked');
                                    $('#delete-all').attr('disabled', 'disabled');
                                })
                                .catch(function (error) {
            ...
            $('#restore-all').click(function () {
                ...
                if (ids.length) {
                    swal({
                        title: '{{ __("page::page.title.Alert") }}',
                        text: "{{ __("page::page.messages.All selected pages will be restored") }}",
                        type: 'warning',
                        showCancelButton: true,
                        confirmButtonColor: '#3085d6',
                        cancelButtonColor: '#d33',
                        ...
                        if (result.value) {
                            axios.post("{{ route('api.page.restore-multiple') }}", {ids: ids})
                                .then(function (rs) {
                                    pageTable.ajax.reload();
                                    notify('{{ __("page::page.title.Notification") }}', '{{ __("page::page.messages.All selected pages were restored") }}', 'success');
                                    $('#mass-select-all').removeAttr('checked');
                                    $('#restore-all').attr('disabled', 'disabled');
                                })
                                .catch(function (error) {
