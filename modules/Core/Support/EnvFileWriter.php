...
class EnvFileWriter
{
    ...
    {
        $environmentFile = $this->finder->get(base_path($this->template));
        $this->finder->put(base_path($this->file), $environmentFile);
    }
    ...
    public function write($vars)
    {
        if (!empty($vars)) {
            ...
            $environmentFile = $this->finder->get($path);
            ...
            $this->finder->put($path, $environmentFile);
        }
    }
}
