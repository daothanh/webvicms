...
class Settings
{
    ...
    {
        $cacheTime = config('core.settings_cache_time');
        if ($cacheTime) {
            $this->setCacheTime(now()->addMinutes($cacheTime)->getTimestamp());
        }
    }
    ...
    public function load($category)
    {
        ...
        $cachedKey = $category . '_' . $this->getCacheId();
        if (Cache::has($cachedKey)) {
            $items = Cache::get($cachedKey);
        } else {
            ...
            Cache::put($cachedKey, $items, $this->getCacheTime());
        }
        if (isset($this->items[$category])) {
    ...
    {
        $result = Setting::query()->where('category', '=', $category)->where('key', '=', $key)->first();
        $value = @serialize($value);
        if (!empty($result)) {
            $result->value = $value;
            $result->save();
        } else {
            $result = Setting::query()->create(array(
                'category' => $category,
                'key' => $key,
                'value' => $value,
            ));
        }
        $cachedKey = $category . '_' . $key.'_'.$this->getCacheId();
        \Cache::put($cachedKey, $value, $this->getCacheTime());
    }
    ...
    public function whenRequestEnds()
    {
        ...
        if (count($this->deleteCategoriesFromDatabase) > 0) {
            foreach ($this->deleteCategoriesFromDatabase as $catName) {
                Setting::query()->where('category', '=', $catName)->delete();
        ...
        if (count($this->deleteItemsFromDatabase) > 0) {
            foreach ($this->deleteItemsFromDatabase as $catName => $keys) {
                Setting::query()->where('category', '=', $catName)->whereIn('key', $keys)->delete();
                $this->cacheNeedsFlush[] = $catName;
            }
        }
