...
class FolderController extends Controller
{
    ...
    public function store(CreateFolderRequest $request)
    {
        ...
        return response()->json([
            'errors' => false,
            'message' => trans('media::media::folders.folder was created'),
            'data' => $folder,
        ]);
    }

    public function update(Media $folder, CreateFolderRequest $request)
    {
        $folder = $this->folder->update($folder, $request->all());

        return response()->json([
            'errors' => false,
            'message' => trans('media::media::folders.folder was updated'),
            'data' => $folder,
        ]);
    }

    public function destroy(Media $folder)
    {
        $this->folder->destroy($folder);

        return response()->json([
            'errors' => false,
            'message' => trans('media::media::messages.folder deleted'),
        ]);
    }
}
