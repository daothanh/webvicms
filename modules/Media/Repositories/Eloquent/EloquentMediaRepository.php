...
class EloquentMediaRepository extends EloquentBaseRepository implements \Modules\Media\Repositories\MediaRepository {
    ...
    public function serverPagingFor(Request $request, $relations = null)
    {
        ...
        if ($request->get('search') !== null) {
            ...
            $media->where(function ($q)  use ($term) {
                $q->where('filename', 'LIKE', "%{$term}%")->orWhere('title', 'LIKE',  "%{$term}%");
            });
        }

        if ($request->get('user_id') !== null) {
            $media->where('user_id', '=', $request->get('user_id'));
        }

        if ($request->get('mimetype') !== null) {
            $media->where('mimetype', '=', $request->get('mimetype'));
        }

        if ($request->get('type') !== null) {
            $type = $request->get('type');
            $media->where('mimetype', 'LIKE', "{$type}/%");
        }

        if ($request->get('extension') !== null) {
            $media->where('extension', '=', $request->get('extension'));
        }
