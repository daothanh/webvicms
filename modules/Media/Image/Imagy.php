...
class Imagy
{
     ...
     * @param  string|Media $originalImage
     * @param  string $thumbnail
     * @return string
     */
    public function getThumbnail($originalImage, $thumbnail)
    {
        if ($originalImage instanceof Media) {
    ...
    private function writeImage($filename, Stream $image)
    {
        ...
        return $this->filesystem->disk($this->getConfiguredFilesystem())->writeStream($filename, $resource, $config);
    }
     ...
     * @return bool
     */
    public function deleteAllFor(Media $file)
    {
    ...
    private function getFilenameFor(MediaPath $path, $thumbnail)
    {
        ...
        $pathPrefix = config('media.files-path');
        if ($filename === false) {
            return $pathPrefix . $this->newFilename($path, $thumbnail);
        }

        return $pathPrefix . $folders . $this->newFilename($path, $thumbnail);
    }
    ...
    {
        $configAssetPath = config('media.files-path');
