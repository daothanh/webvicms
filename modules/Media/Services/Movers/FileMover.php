...
final class FileMover implements MoverInterface
{
    ...
    private function moveDatabase(Media $file, Media $destination) : Media
    {
    ...
    private function getNewPathFor(string $filename, int $folderId)
    {
        ...
        return config('media.files-path') . $filename;
    }
    ...
    private function getFilenameFor(string $path, $thumbnail)
    {
        ...
        if ($filename === false) {
            return config('media.files-path') . $this->newFilename($path, $thumbnail);
        }

        return config('media.files-path') . $folders . $this->newFilename($path, $thumbnail);
    }
