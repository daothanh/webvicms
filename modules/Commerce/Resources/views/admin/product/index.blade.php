    ...
    <div class="card">
        <div class="card-header">
            ...
            <div class="card-tools">
                <a href="{{ route('admin.commerce.product.create') }}" class="btn"><i
                        class="icon ion-md-add"></i> {{ __('commerce::product.title.Create a product') }}</a>
            </div>
        </div>
        <div class="card-body">
    ...
    <script>
      $(function () {
        var productTable = $('#data-table').DataTable({
          ...
          serverSide: true,
          ajax: {
            url: "{{ route('api.commerce.product.index') }}",
            data: function (data) {
        ...
        $('body').on('click', '.del-checkbox', function () {
          ...
          axios.post('{{ route("api.commerce.product.toggle_status") }}', {id: $(this).data('id')});
        });
        ...
        $('#delete-all').click(function () {
          ...
          if (ids.length) {
            swal({
              title: '{{ __("Are you sure?") }}',
              text: "{{ __("All selected products will be moved to the trash!") }}",
              type: 'warning',
              showCancelButton: true,
              confirmButtonColor: '#3085d6',
              cancelButtonColor: '#d33',
              ...
              if (result.value) {
                axios.post("{{ route('api.commerce.product.delete-multiple') }}", {ids: ids})
                  .then(function (rs) {
                    productTable.ajax.reload();
                    notify('{{ __("Notification") }}', '{{ __("All selected products were moved to the trash!") }}', 'success');
                  })
                  .catch(function (error) {
                    notify('{{ __("Alert") }}', error.response.data.message, 'error');
                  });
        ...
        $('#restore-all').click(function () {
          ...
          if (ids.length) {
            swal({
              title: '{{ __("Are you sure?") }}',
              text: "{{ __("All selected products will be restored!") }}",
              type: 'warning',
              showCancelButton: true,
              confirmButtonColor: '#3085d6',
              cancelButtonColor: '#d33',
              ...
              if (result.value) {
                axios.post("{{ route('api.commerce.product.restore-multiple') }}", {ids: ids})
                  .then(function (rs) {
                    productTable.ajax.reload();
                    notify('{{ __("Notification") }}', '{{ __("All selected products were restored!") }}', 'success');
                  })
                  .catch(function (error) {
                    notify('{{ __("Alert") }}', error.response.data.message, 'error');
                  });
