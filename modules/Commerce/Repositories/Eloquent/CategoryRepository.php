    ...
    public function serverPagingFor(Request $request, $relations = null)
    {
        ...
        if ($request->get('order_by') !== null && $request->get('order') !== 'null') {
            ...
            if (Str::contains($request->get('order_by'), '.')) {
                ...
                $query->with('translations')->join('commerce__category_translations as t', function ($join) {
                    $join->on('commerce__categories.id', '=', 't.category_id');
                })
                    ->where('t.locale', \App::getLocale())
                    ->groupBy('commerce__categories.id')->orderBy("t.{$fields[1]}", $order);
            } else {
